...
from .geometry import distance, visible_isl, elevation_deg, C_KM_S


def candidate_pairs(objects, constellations, topology):
    sats = [o for o in objects if o["kind"] == "satellite"]
    if topology == "distance":
        return sorted(tuple(sorted((a["id"], b["id"]))) for a, b in combinations(sats, 2))
    ...
    for c in constellations:
        ...
        members = {(o["plane"], o["slot"]): o["id"] for o in sats if o["constellation_id"] == c["id"]}
        for (p, s), sid in members.items():
            # Forward edges represent both directions in this undirected graph.
            for target in (members[(p, (s+1) % slots)], members[((p+1) % planes, s)]):
                if target != sid:
...
def build_links(objects, constellations, states, policy):
    ...
    for a, b in candidate_pairs(objects, constellations, policy["topology"]):
        ...
        if sa.enabled and sb.enabled and distance(sa.position_ecef_km, sb.position_ecef_km) <= policy["max_isl_km"] and visible_isl(sa.position_ecef_km, sb.position_ecef_km):
            add(a, b, "isl")
    for gs in (o for o in objects if o["kind"] == "ground_station"):
        g = states[gs["id"]]
        for sat in (o for o in objects if o["kind"] == "satellite"):
            s = states[sat["id"]]
            if g.enabled and s.enabled and elevation_deg(g.position_ecef_km, s.position_ecef_km) >= policy["min_elevation_deg"]:
                add(g.id, s.id, "gsl")
