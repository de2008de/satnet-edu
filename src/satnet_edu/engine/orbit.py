...
def generate_tle(satnum, epoch, altitude_km, inclination_deg, raan_deg, phase_deg):
    period = 2 * math.pi * math.sqrt((R_earth.value + altitude_km * 1000) ** 3 / (G.value * M_earth.value))
    ...
    sat = Satrec()
    sat.sgp4init(WGS72, "i", satnum, jd + fr - 2433281.5, 0.0, 0.0, 0.0,
                 1e-7, 0.0, math.radians(inclination_deg), math.radians(phase_deg),
                 motion, math.radians(raan_deg))
...
def propagate(tle, epoch, t_s):
    ...
    p = (EARTH_RADIUS_KM + r * math.sin(alt), r * math.cos(alt) * math.sin(az),
