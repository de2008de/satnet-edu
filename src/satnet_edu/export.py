"""Safe single-file packing; never imports or invokes the simulation engine."""
import json
...
def export_html(trace,path,language='en'):
    if language not in ('en','zh'): raise ValueError('language: expected en or zh')
    validate_trace(trace)
    payload=safe_json(trace)
    if len(payload.encode('utf-8'))>DEFAULT_MAX_BYTES: raise ValueError('trace exceeds offline export size limit')
    script=files('satnet_edu').joinpath('assets/satnet-edu-player.js').read_text(encoding='utf-8')
    html=f'''<!doctype html>
...
player.load(JSON.parse(document.getElementById('trace-data').textContent)).catch(()=>{{}});
    ...
    path=Path(path);path.parent.mkdir(parents=True,exist_ok=True);path.write_text(html,encoding='utf-8');return path.resolve()
