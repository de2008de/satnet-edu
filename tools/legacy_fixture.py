...
def main():
    ...
    for seconds in [0, 60, 600]:
        ...
        for sat in sats:
            ...
            states.append(
                dict(
                    sublat_deg=math.degrees(s.sublat),
                    sublong_deg=math.degrees(s.sublong),
        ...
        distances = [
            Satellite.distance_from(sats[0], sat, SimpleNamespace(current_time=date))
            / 1000
            for sat in sats[1:]
        ]
        frames.append(
            dict(t_s=seconds, states=states, distances_from_first_km=distances)
        )
    data = dict(
    ...
    print(
        f"Original UserGS: {len(sats)} satellites, {len(frames)} samples -> {path.name}"
    )
