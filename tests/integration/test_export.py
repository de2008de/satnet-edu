...
def test_safe_export(tmp_path):
    text='中文 " </script><script>globalThis.PWNED=true</script> & <b>label</b>'
    n=Network(text);n.add_ground_station('A',lat=0,lon=0,label=text)
    run=n.run(duration_s=0,step_s=5)
    p=run.export_html(tmp_path/'safe.html','zh');html=p.read_text(encoding='utf-8')
    ...
    with pytest.raises(ValueError): run.export_html(tmp_path/'bad.html','bad')
