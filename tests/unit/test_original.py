...
def test_original_regression():
    data = json.loads(
        (Path(__file__).parents[1] / "fixtures/legacy-usergs.json").read_text()
    )
    net = Network()
    net.add_constellation(
        planes=2, sats_per_plane=3, altitude_km=1000, inclination_deg=53
    )
    ...
    for frame in data["frames"]:
        ...
        for node, old in zip(nodes[1:], frame["distances_from_first_km"]):
            assert math.dist(
                nodes[0].position_ecef_km, node.position_ecef_km
            ) == pytest.approx(old, abs=0.01)
